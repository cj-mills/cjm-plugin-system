...
- **CR-8** `compute_config_schema_hash` — hashes a plugin’s JSON Schema
  for drift detection (manifest-stored vs live worker).
- **CR-7** `compute_config_hash` — hashes a plugin instance’s effective
  config to key `EmpiricalResourceRecord` storage by (instance_id,
  config_hash).
