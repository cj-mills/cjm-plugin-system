...
down plugin execution — handlers swallow; the journal’s loud rule
...
never break plugin execution (the journal’s loud rule is the journal’s
alone).
...
Thread-safe via per-call connections (the worker runs plugin execute in
an executor thread; contextvars propagate via copy_context at the
...
from cjm_plugin_system.core.wire import CallEnvelope, set_call_envelope, reset_call_envelope
