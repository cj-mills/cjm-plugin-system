...
`PluginManager` satisfies this structurally, so existing hosts continue
to work — passing a `PluginManager` to `JobQueue(deps=...)` still
type-checks. The Protocol enables clean future extraction of JobQueue
into a separate library per OQ-5 resolution. The audit’s locked design
listed 3 methods; investigation surfaced 2 more (`get_plugin`,
`reload_plugin`, `get_plugin_logs`) that are consumed in the current
execute path. All 5 are included so the Protocol genuinely captures the
surface the queue uses.
...
PluginManager satisfies this structurally; the Protocol exists so
...
queue isn’t constructed with explicit stores — a deps without them (test
doubles) simply yields no journaling.

## Job / JobEvent / QueueStats / \_Subscription

`Job` is the queued execution unit (CR-6 reshape: `plugin_instance_id`
per CR-10, `JobError` typed error per CR-5, datetime timestamps; stage 3
replaced the sequence tags with composition tags —
`composition_id`/`node_id` — alongside the cancel-phase + block-reason +
retry fields). `JobEvent` is the push-bus payload — every event carries
full tag context (`job_id`, optional `composition_id`/`node_id`,
`plugin_instance_id`) so consumers subscribed to any filter view get the
same shape. `QueueStats` is the aggregate-counts shape returned by
`get_stats()`. `_Subscription` is the internal per-subscriber
bounded-queue + drop-counter holder.

`Job` keeps backward-compat `@property` aliases for `plugin_name`
(returns `plugin_instance_id`) and `created_at` (returns
...
def JobEvent(
    type:JobEventType, job_id:str, plugin_instance_id:str, composition_id:Optional=None, node_id:Optional=None,
...
def Job(
    ...
    cancel_requested_at:Optional=None, cancel_phase:Optional=None, block_reason:Optional=None, retry_count:int=0,
    last_resource_snapshot:Optional=None
)->None:
...
system-monitor plugin’s CR-3 typed `get_system_status()` +
...
`manager: PluginManager` (concrete); adds bounded-queue multi-subscriber
event-bus state.
...
async def submit(
    ...
    run_id:Optional=None, # Host-tier run correlation (CR-14 follow-up; reserved name, never a plugin kwarg)
...
CR-6 splits the legacy `get_state()` UI-shaped dict into four typed
accessors: `get_pending()`, `get_running()`, `get_history(limit)`,
`get_stats()`. The legacy `get_state()` remains as a
REMOVE-AFTER-OVERHAUL shim (see the shim cell below this section) so
existing UI consumers continue to work until SG-48 sweeps them.
...
the deleted `_slice_log_by_job_window` heuristic got wrong).
...
the configured sysmon plugin’s CR-3 typed `list_processes()` (per-PID
matching) and `get_system_status()` (global GPU stats).
...
(typically a PluginManager). When CR-7’s reactive-retry path fires for a
running job, the observer updates `Job.retry_count` and publishes a
...
The pre-CR-6 dict shape that existing UI consumers (notably the
`cjm-fasthtml-job-monitor` library) read today. Reshape into typed
accessors (`get_pending` / `get_running` / `get_history` / `get_stats`)
is canonical; this shim converts those back to the legacy dict shape
until the consumer cascade migrates. SG-48 removes it.
...
# executor routing task-addressed jobs via execute_plugin_task_async while
# execute-channel jobs stay on execute_plugin_async.
...
class _TaskChannelDeps:
    ...
    async def execute_plugin_task_async(self, name_or_id, task_name, method, **kwargs):
