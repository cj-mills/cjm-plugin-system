...
```
    ...
    Temp file path: /tmp/tmpdnir0zfz.wav
...
- **Encode is exact-type**: only DTO classes registered via `@wire_type`
  ...
  plugins, primitives, unregistered dataclasses) passes through to the
...
reaches the worker only as a domain kwarg that polite plugin authors
...
- **Host side**: the queue sets the contextvar around each job
  ...
  `kwargs` — plugin code never sees it; old workers ignore unknown
  top-level keys).
...
contract (typed wire envelope / plugin dict) — wrapping it would break
any old host talking to a new worker. A header is invisible to old
...
plugin cooperation) and the interface-lib storage helpers (T29
`save_with_logging` → RESULT_SAVED, `get_cached` → CACHE_HIT — recorded
at the helper boundary, so every plugin using the helpers reports for
free). `record_account` outside a capture span is a silent no-op
