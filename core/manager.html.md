...
- **Discovery**: Finds plugin manifests in local (`.cjm/plugins/`) or
  global (`~/.cjm/plugins/`) directories
- **Loading**: Creates
...
def PluginManager(
    plugin_interface:Type=<class 'cjm_plugin_system.core.interface.PluginInterface'>, # Base interface for type checking
...
- **Local-first discovery**: Manifests in `.cjm/plugins/` shadow global
  ones in `~/.cjm/plugins/`
